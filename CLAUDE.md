...
- **Dev/test** runs on port **4141**. **Prod** runs on port **4142** (at `C:\jason\dev\prod`).

Use these npm scripts to manage it:

- **Start (dev):** `PORT=4141 npx ts-node -r tsconfig-paths/register src/main.ts &` (background) or `npm run start:dev` (watch mode, port 4141)
- **Stop (dev):** `npm run stop-service` — kills whatever process is listening on port 4141
- **Stop (prod):** `npm run stop-service-prod` — kills whatever process is listening on port 4142
...
```
powershell -Command "Stop-Process -Id (Get-NetTCPConnection -LocalPort 4141 -State Listen).OwningProcess -Force"
```
...
Integration tests run against the **dev server on port 4141** (not prod). Before running `npm run test:integration`, start the dev server:

```
PORT=4141 npx ts-node -r tsconfig-paths/register src/main.ts &
```

The default `PROXY_URL` in the integration tests is `http://localhost:4141`. Tests also require llama.cpp running on port 8080 (`LLAMA_BASE_URL`, default `http://localhost:8080`).
